    ...
    <div className='mt-8 pt-5 border-t border-white/10'>
      ...
      <div className='flex gap-4 overflow-x-auto scrollbar-hide pb-4 snap-x snap-mandatory scroll-smooth'>
        {screenshots.map((src) => (
          <div
            key={src.id}
            onClick={() => setSelectedImg(src.image)}
            className='shrink-0 w-[320px] md:w-[420px] lg:w-[500px] aspect-video rounded-xl overflow-hidden border border-white/5 cursor-pointer group transition-all duration-300 hover:border-white/15'
          >
            <img
      ...
      {selectedImg && (
        ...
        >
          ...
          />
          <button className="absolute top-6 right-6 text-white/60 hover:text-white text-2xl transition-colors">
...
const GameContent = ({ game,screenshots,stores,similarGames,achievements }) => {
  ...
  return (
    <div className='flex-1 flex flex-col gap-6 rounded-3xl h-full overflow-x-hidden overflow-y-auto pb-10' style={{ overflowAnchor: 'none' }}>
